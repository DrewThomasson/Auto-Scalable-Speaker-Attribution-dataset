...
The [exhaustive maximum-accuracy benchmark](benchmark-results/booknlp-local-llm-exhaustive-2026/README.md) tests a different inference strategy: software asks a separate local model question for every token, then adds independent wider-context votes, span verification, pairwise coreference decisions, and consistency constraints. It reuses the same frozen gold cohorts and scorers, adds Qwen3.5 0.8B, and keeps all original results above unchanged. The first complete held-out result is Qwen3.5 0.8B events: **30/30 excerpts, P 0.0395 / R 0.6987 / F1 0.0748** versus BookNLP-small F1 **0.7036** on the same excerpts. The model made 127,577 local calls over 12.43 hours; high recall came with extreme overprediction, and the uncalibrated top-25%-coverage subset had only 0.0365 precision. This does not show that the 0.8B model is a useful event teacher. Other exhaustive model/task cohorts are still running; no same-model old-vs-new conclusion is available yet. See the [full status, comparison, and ablations](benchmark-results/booknlp-local-llm-exhaustive-2026/RESULTS.md), [comparison CSV](benchmark-results/booknlp-local-llm-exhaustive-2026/results/comparison.csv), [machine-readable cohort summary](benchmark-results/booknlp-local-llm-exhaustive-2026/results/summaries/qwen3.5_0.8b_test_events.json), and separate [validation pilot](benchmark-results/booknlp-local-llm-exhaustive-2026/results/validation_pilot_qwen08_events.json).
