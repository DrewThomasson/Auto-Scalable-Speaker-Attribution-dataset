"""Run the frozen BookNLP-style test cohorts against DeepSeek V4.1 Flash.
...
MODEL_URL = "https://openrouter.ai/api/v1/models/deepseek/deepseek-v4.1-flash/endpoints"
...
def fetch_model_metadata() -> dict[str, Any]:
    ...
    endpoint = next((item for item in endpoints
                     if item.get("provider_name", "").casefold() == PROVIDER.casefold()), None)
    if endpoint is None:
        raise RuntimeError(f"OpenRouter has no {PROVIDER} endpoint for {MODEL}")
    supported = set(endpoint.get("supported_parameters", []))
    required = {"max_tokens", "temperature", "reasoning_effort", "response_format"}
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    output_root = Path(args.output_dir).resolve() if args.output_dir else experiment_root / "results" / "openrouter_deepseek_v4_1_flash"
    ...
    config = {
        ...
        "provider_context_length": model_metadata["selected_provider_endpoint"].get("context_length"),
        "context_limit_note": "OpenRouter does not expose an Ollama-style num_ctx setting; the pinned endpoint advertises its provider context length. The complete unchunked prompts are sent as in the baseline.",
            ...
            if cached is None:
                ...
                cached = {
                    ...
                    "usage": usage, "response": {
                        **{key: response.get(key) for key in ("id", "model", "provider", "created")
                           if key in response},
                        "choices": choices,
                        "usage": usage,
                    },
