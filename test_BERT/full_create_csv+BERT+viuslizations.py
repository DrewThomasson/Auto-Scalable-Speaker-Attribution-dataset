...
# Training arguments with save_steps and evaluation_steps adjusted
...
# For visualization, we'll need to store evaluation results after each evaluation
eval_results = []
...
def train_and_evaluate():
    ...
    accuracy_scores = [result['eval_accuracy'] for result in eval_results]
    plt.plot(range(len(accuracy_scores)), accuracy_scores, marker='o')
    plt.xlabel('Evaluation Step')
    plt.ylabel('Accuracy')
    plt.title('Model Evaluation Accuracy Over Time')
    plt.show()

train_and_evaluate()

# Continue from the train_and_evaluate function

def train_and_evaluate():
    # Train the model
    trainer.train()
    
    # Evaluate the model on the validation set after training completes
    final_metrics = trainer.evaluate()
    print(final_metrics)
    
    # Plotting the evaluation results (if needed, as mentioned previously)
    
    # You can insert the plotting code here if you're collecting evaluation metrics
...
# Call the function to train, evaluate, and predict
train_and_evaluate()
